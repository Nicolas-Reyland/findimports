...
class ModuleGraph(object):
    ...
    def printImportsPip(self):
        """Produce a report of dependencies in the pip-format."""
        # print("# Generated by: https://github.com/mgedmin/findimports/")
        for module in self.listModules():
            print(f"# {module.label}:")
            if self.external_dependencies:
                imports = list(module.imports)
...
def main(argv=None):
    ...
    parser.add_option('-P', '--pip-format', action='store_true',
                      dest='pip_format',
                      help="Format the output so that it can be redirected to"
                           " a pip-format requirements.txt file")
